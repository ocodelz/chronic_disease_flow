<template>
	<scroll-view class="wrap" scroll-y>
		<view v-if="state == '0'">
			<free-title title="档案列表"></free-title>
			<view class="container">
				<view class="content">
					...
					<view class="main" v-for="(item,index) in list" :key="index">
						...
						<text class="txt" v-if="item.item == '出生日期' || item.item == '建档日期'">-</text>
						<input v-if="item.item == '出生日期' || item.item == '建档日期'" :type="item.type"
							@click="item.type == 'select' ? handleTapInput(item,1) : ''" :placeholder="item.placeholder"
							:disabled="item.disabled" v-model="item.mode1" />
					</view>
					<view class="action">
						<text class="item">拍照</text>
					...
					<view class="table-box" v-if="!table.length">
						<view class="table-th">
							...
							<view class="item">档案号</view>
							<view class="item">出生日期</view>
							<view class="item">人群类型</view>
							<view class="item">建档日期</view>
							<view class="item">责任医生</view>
						</view>
						<scroll-view scroll-y class="scroll">
							<view style="display: flex;align-items: center;justify-content: center; padding-top: .2rem;">
					...
					<view class="scroll-table" v-if="table.length">
						<scroll-view class="scroll-table-container" scroll-x :show-scrollbar="true">
							<view class="scroll-table-content">
								...
								<view class="scroll-table-item">年龄</view>
								<view class="scroll-table-item">档案号</view>
								<view class="scroll-table-item">出生日期</view>
								<view class="scroll-table-item">人群类型</view>
								<view class="scroll-table-item">建档日期</view>
								<view class="scroll-table-item">责任医生</view>
								<view class="scroll-table-item">是否拍照</view>
							...
							<view v-for="(item,index) in table" :key="index" class="scroll-table-body">
								...
								<view class="scroll-table-body-item">{{item.age}}</view>
								<view class="scroll-table-body-item">{{item.person_id}}</view>
								<view class="scroll-table-body-item">{{item.birthdate}}</view>
								<view class="scroll-table-body-item">{{item.person_type}}</view>
								<view class="scroll-table-body-item">{{item.create_time}}</view>
								<view class="scroll-table-body-item">{{item.docname}}</view>
								<view class="scroll-table-body-item">{{item.picture_status}}</view>
								<view class="scroll-table-body-item">{{item.upload_status}}</view>
							</view>
						</scroll-view>
						<!-- 操作 -->
						<view style="width: 30%;font-size: .12rem;">
							...
							<view v-for="(item,index) in table" :key="index"
								...
								<view
									...
									@click="handleTableEdit(item)">编辑</view>
								<view
									style="background-color: #ff5722;border-radius: 8rpx;padding: 10rpx;margin-left: .1rem;"
									@click="handleTapDelCell(item.person_id)">删除</view>
							</view>
						</view>
					</view>
				</view>
		...
		<u-popup v-model="isPersonalFiles" mode="center" width="90%" border-radius="16">
			<personal-files v-if="isPersonalFiles == true" :file="file"></personal-files>
		</u-popup>
...
<script>
	...
	import data from '@/js/fileQuery.js'
	export default {
		components: {
			freeTitle,
			personalFiles
		...
		methods: {
			...
			handleTapSearch() {
				...
				for (let item of this.list) {
					...
					if (item.item == '人群类型') {
						obj.person_type = item.model
					}
				}
				console.log(obj);

				this.$u.post('SearchArchivesList', obj).then(res => {
					console.log(res.data.infoList);
					if (res.code == 200 && res.info == '响应成功' && res.data.infoList.length) {
						for (let a of res.data.infoList) {
							a.checked = false;
						}
			...
			handleChangeSwitch(e) {
				this.picture_status = e == true ? '1' : '0'
			},
			// 删除table列表
			handleTapDelCell(item) {
				console.log(item);
				this.$u.post('DelPersonInfo', {
					person_id: item
				}).then(res => {
					this.$lz.toast(res.info);
					this.handleTapSearch();
				}).catch(err => {
					this.$lz.toast(res.err);
				})
			},
			//  
			...
			handleTapInput(item, index) {
				...
				if (item.item == '出生日期' || item.item == '建档日期') {
					this.isTime = true;
				}
				if (item.item == '人群类型') {
